...
const tysonProfile = {
	...
	favouriteMusic: {
		...
		ledZeppelin: {
			...
			reasonHeLikes:
				'The raw energy and the way they blend blues and rock',
		},
		theBeatles: {
	...
	favouriteMovies: {
		...
		theDarkKnight: {
			reasonHeLikes:
				'The way it shows the struggle between good and evil',
			favouriteQuotes: [
	...
	familyMembers: {
		parents: [
			'Stacey is tysons mom, Batfink, and Hugo are her dogs',
			'Geoff',
		],
		siblings: ['Jordi'],
		wife: ['Katelyn'],
	...
	career: {
		...
		currentFocus: [
			'Currently working at id8r.com as a fullstack developer with a Three.js specialty, and previously built TaiL-Adventures.com, a dynamic storytelling platform using Ai connected to backened database. Tyson is also exploring 3D in the browser and AR technologies. He has a previous background in managing large and small teams, as a General Manager and culture leader at Indigo Books, and HMV.',
		],
	},
};
...
export async function POST(req: Request) {
			...
			return Object.entries(movies)
				.map(([key, value]) => {
					return `${key} for the reason ${
						value.reasonHeLikes
					} and quotes like ${arrayToReadableList(
    ...
    He supports open source and is keen on ${arrayToReadableList(
		...
		)}, with his current focuses including ${arrayToReadableList(
			profile.career.currentFocus
		)}, His musical tastes include ${serializeMusicPreferences(
